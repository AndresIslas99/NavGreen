...
# A cold boot in a clock-isolated greenhouse (no internet, router not
# advertising NTP yet) causes systemd-timesyncd to adjust the wall clock
# asynchronously DURING the ROS stack startup. Every TF lookup that bridges
# the jump rejects messages as TF_OLD_DATA, the dual EKF discards inputs,
# and the orchestrator declares localization FAILED. Observable as "robot
# offline" with no clear cause in journalctl.
#
...
# downstream effects). Recommended router config in docs/hardware_setup.md
# — set the LAN router as the NTP server.
