...
[`src/`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src)**, plus
the optional fleet layer in
[`fleet/`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/fleet) and
the operator dashboard in
[`web/`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/web).
...
!!! note "How to read the CI column"
    CI ([`.github/workflows/ci.yaml`](https://github.com/AndresIslas99/agv-greenhouse/blob/main/.github/workflows/ci.yaml))
...
| [`agv_description`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_description) | URDF/Xacro robot model, TF chain, sensor mounts; nominal geometry in `config/robot_params.yaml` | `robot_state_publisher` owns `base_link → children` TF | Built in CI (also in the `simulation` job) |
| [`agv_bringup`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_bringup) | Launch orchestration — owns the three entry points: `agv_full.launch.py`, `agv_mapping.launch.py`, `agv_hil_full.launch.py` | Startup DAG specified in [`specs/launch_sequence.yaml`](https://github.com/AndresIslas99/agv-greenhouse/blob/main/specs/launch_sequence.yaml) | **Not built in CI** — declares all three vendor-SDK packages as runtime deps. The full stack also requires the external `agv_slam` (cuVSLAM) overlay, which is not published |
...
| [`agv_zone_detector`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_zone_detector) | Classifies the robot pose into greenhouse zones (corridor vs rail aisle) so Nav2 is never used inside rail aisles | Pub: `/agv/zone/state` (JSON, 10 Hz) | Built + tested in CI |

## Navigation & missions

| Package | Purpose | Key interfaces | Status / notes |
|---|---|---|---|
| [`agv_navigation`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_navigation) | Nav2 configuration and launch: planning, MPPI control, collision monitor | Nav2 output on `/agv/cmd_vel_nav` (Phase 2 launches) or `/agv/cmd_vel` (legacy); collision monitor emits `/agv/cmd_vel_collision_safe` | Built in CI |
...
| [`agv_behaviors`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_behaviors) | BehaviorTree.CPP mission execution | Behavior trees under `trees/` | Built + tested in CI. One of two mission-execution paths — consolidation tracked in [#7](https://github.com/AndresIslas99/agv-greenhouse/issues/7) |
| [`agv_waypoint_manager`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/src/agv_waypoint_manager) | Mission CRUD (storage) + sequential waypoint dispatch | Calls `/agv/navigate_to_pose` directly (documented `known_gap`: bypasses the dashboard's dispatch gates) | Built + tested in CI. See [#7](https://github.com/AndresIslas99/agv-greenhouse/issues/7) |
...
[`fleet/README.md`](https://github.com/AndresIslas99/agv-greenhouse/blob/main/fleet/README.md).

| Package | Purpose | Key interfaces | Status / notes |
|---|---|---|---|
| [`fleet/agv_fleet_manager`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/fleet/agv_fleet_manager) | VDA 5050 master: fleet state aggregation, order dispatch, traffic zones | REST + WebSocket on `:8092` (`FLEET_PORT`); token auth via `FLEET_API_TOKEN` | TypeScript. Built + vitest in CI |
...
| [`fleet/mosquitto`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/fleet/mosquitto) | MQTT broker configuration (VDA 5050 backbone, ports 1883/9001) | Ships `allow_anonymous true`; enable procedure documented in `mosquitto.conf` | Config only — read [Security](../community/security.md) before deploying |
| [`fleet/systemd`](https://github.com/AndresIslas99/agv-greenhouse/tree/main/fleet/systemd) | systemd service units for broker, fleet manager, and adapter | `agv-mosquitto.service`, `agv-fleet-manager.service`, `agv-vda5050-adapter.service` | Config only |
