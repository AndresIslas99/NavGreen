...
**[roadmap tracking issue #18](https://github.com/AndresIslas99/NavGreen/issues/18)**.

!!! note "GitHub is the live source"
    This page is a snapshot (July 2026). Issue states, discussion, and new
    items live on the
    [issue tracker](https://github.com/AndresIslas99/NavGreen/issues).
...
- [#6 — Publish a runnable simulation story](https://github.com/AndresIslas99/NavGreen/issues/6)
...
- [#20 — Make the headless Gazebo controller smoke test blocking in CI](https://github.com/AndresIslas99/NavGreen/issues/20)
...
- [#7 — Consolidate the two mission-execution subsystems](https://github.com/AndresIslas99/NavGreen/issues/7)
  — the TypeScript executor in `agv_ui_backend` and the C++
  `agv_behaviors`/`agv_waypoint_manager` stack both exist; one owner should
  win.
- [#8 — Honor `robot_namespace` instead of hardcoding `agv`](https://github.com/AndresIslas99/NavGreen/issues/8)
  — the canonical namespace parameter is currently decorative, which blocks
  multi-robot deployments.
- [#9 — Extract the duplicated CAN/ODrive protocol layer](https://github.com/AndresIslas99/NavGreen/issues/9)
  — `agv_odrive` and `agv_hw_interface` copy-paste the SocketCAN/ODrive
  code; it should be one shared library package.
- [#10 — Make `agv_factor_graph` an actually-independent estimator](https://github.com/AndresIslas99/NavGreen/issues/10)
  — it is pitched as independent validation of `ekf_global` but consumes
  `ekf_global`'s own output.

## Test expansion

- [#11 — TypeScript tests for the UI backend and VDA 5050 adapter](https://github.com/AndresIslas99/NavGreen/issues/11)
...
- [#12 — Unit tests for `agv_factor_graph` and `agv_localization_init`](https://github.com/AndresIslas99/NavGreen/issues/12)
...
- [#13 — Optional CI job that builds the vendor-SDK packages](https://github.com/AndresIslas99/NavGreen/issues/13)
...
- [#15 — Atomic map/zone/mission writes + dashboard sidecar parity](https://github.com/AndresIslas99/NavGreen/issues/15)
- [#16 — Strongly-typed collision-monitor subscription in `agv_rail_driver`](https://github.com/AndresIslas99/NavGreen/issues/16)
- [#17 — Move the JWT out of the WebSocket URL query parameter](https://github.com/AndresIslas99/NavGreen/issues/17)

## Polish & first impressions

- [#14 — Dashboard/HIL demo GIF in the README](https://github.com/AndresIslas99/NavGreen/issues/14)
- [#19 — Clear the dashboard's pre-existing eslint errors](https://github.com/AndresIslas99/NavGreen/issues/19)

## Owner-only items

A few items from the review can only be done by the repository owner and are
listed in [#18](https://github.com/AndresIslas99/NavGreen/issues/18)
and
[`docs/going_public_checklist.md`](https://github.com/AndresIslas99/NavGreen/blob/main/docs/going_public_checklist.md):
...
[`help wanted`](https://github.com/AndresIslas99/NavGreen/labels/help%20wanted)
or
