...
src/agv_bringup/config/
  ...
  greenhouse/     # site-specific (example/, future sites)
  simulation/     # sim-only overrides
  hil/            # hardware-in-the-loop overrides
```

A launch argument `site:=<name>` (default `example`) selects which `greenhouse/<name>/` overlay to merge on top of `common/` and `robot/`. Per-package configs still exist for things that are genuinely package-internal — the goal is centralization of *deployment* parameters, not all parameters.
