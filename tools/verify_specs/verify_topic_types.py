...
def parse_cpp_file(path: Path) -> list[tuple[str, str, str, str, int]]:
    ...
    for m in _CREATE_RE.finditer(text):
        ...
        # Find line number
        line = text.count("\n", 0, m.start()) + 1
        out.append((topic, cpp_type_to_spec(cpp_type), kind, str(path.relative_to(WS_ROOT)), line))
    return out
...
def main() -> int:
    ...
    decls: list[tuple[str, str, str, str, int]] = []
    ...
    for topic, code_type, kind, file, line in decls:
