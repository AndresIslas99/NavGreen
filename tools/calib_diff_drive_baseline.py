#!/usr/bin/env python3
"""Diff-drive baseline characterization harness (operator drives, script measures).

Auto-watch tool: the operator drives the robot freely with the dashboard
joystick. This script watches /agv/wheel_odom for motion and captures
Δtag (AprilTag) vs Δodom (wheel encoders) for each "leg" (motion-then-stop
cycle). Designed as the reproducible baseline that gates every change to
the odometry stack — see docs/calibration/baseline_protocol.md.

Flow per leg:
  IDLE        → robot still: keep updated reservoir of recent tag poses
  MOVING      → |wheel velocity| > velocity_threshold sustained
                 (use the most recent reservoir as PRE-pose)
  SETTLING    → velocity dropped below threshold, wait settle_s
  CAPTURED    → take fresh median POST-pose; print delta; back to IDLE

The CSV header includes session metadata (surface, payload_kg, tag_id,
timestamp, git_sha) so historical baselines can be diff'd. If --record-bag
is passed, a parallel rosbag2 recording captures the underlying ROS topics
for offline analysis.

Stop with Ctrl+C. CSV + summary are written on exit.
...
Usage:
  ...
  python3 tools/calib_diff_drive_baseline.py \
      --surface "polished ceramic" --payload-kg 0 --record-bag
"""
...
def start_bag_record(out_path: Path) -> subprocess.Popen | None:
    """Start a background rosbag2 recording. The set of topics is fixed
    by the baseline protocol so historical bags are comparable."""
    topics = [
        "/agv/wheel_odom",
        "/agv/odometry/local",
        "/agv/odometry/global",
    ...
    cmd = ["ros2", "bag", "record", "-s", "mcap", "-o", str(out_path)] + topics
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--surface", type=str, default="unspecified",
                        help="floor surface description, e.g. 'polished ceramic', 'concrete', 'rubber mat'")
    parser.add_argument("--payload-kg", type=float, default=0.0,
                        help="extra payload on the AGV during this session")
    parser.add_argument("--label", type=str, default="",
                        help="free-form label appended to the CSV filename")
    parser.add_argument("--record-bag", action="store_true",
                        help="run rosbag2 in parallel to capture full topic state")
    ...
    if args.record_bag:
        bag_path = Path(args.out_dir) / f"bag_{session_ts}"
        bag_proc = start_bag_record(bag_path)
        print(f"rosbag2 recording → {bag_path}")
    ...
    try:
        ...
        if bag_proc is not None:
            try:
                bag_proc.send_signal(2)  # SIGINT, lets rosbag2 close cleanly
        ...
        csv_path = Path(args.out_dir) / f"baseline_{session_ts}{label}.csv"
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f)
            # Session metadata as a YAML-ish header so a CSV diff highlights changes
            w.writerow(["# session_ts", session_ts])
            w.writerow(["# git_sha", git_sha])
            w.writerow(["# surface", args.surface])
            w.writerow(["# payload_kg", f"{args.payload_kg:.2f}"])
            w.writerow(["# tag_id", args.id])
            w.writerow(["# tag_size_m", f"{args.tag_size:.3f}"])
            w.writerow(["# velocity_threshold_mps", f"{args.velocity_threshold:.3f}"])
            w.writerow(["# settle_s", f"{args.settle_s:.2f}"])
            w.writerow(["# min_leg_distance_m", f"{args.min_leg_distance:.3f}"])
            w.writerow([])
            w.writerow([
                "leg", "dt_s",
